...
### 1b. 纯本地检测（`detect_source_local`，0 GitHub API）

上面三信号原本绑死在 `check_upstream_status` 里——每个信号拿到 repo 后立刻调 GitHub API 判版本，所以扫描默认（不含 upstream check）时根本不检测来源，「待补来源」页没数据。

`detect_source_local`（`source_ops.py`）把三信号读取抽成纯本地函数：只读 `.skill-source.env` / `.git origin` / `.skill-lock.json`，命中第一个就停，**绝不调网络**。返回 `{source, repo, ref, subdir}`，`source ∈ steal-meta / git-remote / vercel-lock / unknown`。

扫描默认就能跑（0 API），结果通过 `source_status` 字段喂给前端「待补来源」tab（见第 7 节第 6 条）。
...
6. ✅ **recover 数据流打通**：`detect_source_local`（纯本地，0 API）→ `/api/scan-run` 响应 `source_status` 字段（每技能 `{name, dir, source, repo}`，**跟随"开始整理"选定范围**，只收 `category=user/project`，排除货架/缓存/vendor/builtin——这些本就没单个来源留痕，也不该让用户补）→ 前端「待补来源」tab 列出 `source=unknown` 的 skill（该 tab 现归「上游检测」视图，已从 issues 页拆出）。范围一致性：与同名/上游/变更 tab 同源（都来自用户勾选的目录），选"当前目录"只看当前目录的缺来源 skill。
