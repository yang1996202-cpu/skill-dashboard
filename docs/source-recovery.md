...
4. ⏳ 按内容 code search 通用层（多片段召回 + hash 确认）—— 未实现
5. ⏳ unknown skill 上"补来源"入口（确认后写 `.skill-source.env`）—— 未实现
...
**对 dashboard 的含义**：dashboard 对 `~/.workbuddy/skills` 等 app 自管目录是**只读旁观**——读 + 展示 OK，但**写（steal 装进去）可能被 app 同步覆盖/清理**（app 不认外部装的 skill；这解释了"合集勾选装成功但 workbuddy 看不到/文件消失"的现象）。dashboard 不该自己实现 buddy family 的更新逻辑（app 已管）。**版本锁是 dashboard 可差异化的点**（buddy family 没覆盖）。
