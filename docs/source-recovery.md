...
6. ✅ **recover 数据流打通**：`detect_source_local`（纯本地，0 API）→ `/api/scan-run` 响应 `source_status` 字段（每技能 `{name, dir, source, repo}`，**跟随"开始整理"选定范围**，只收 `category=user/project`，排除货架/缓存/vendor/builtin——这些本就没单个来源留痕，也不该让用户补）→ 前端「待补来源」tab 列出 `source=unknown` 的 skill。范围一致性：与同名/上游/变更 tab 同源（都来自用户勾选的目录），选"当前目录"只看当前目录的缺来源 skill。
