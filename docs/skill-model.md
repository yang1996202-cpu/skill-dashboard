...
5. ✅ 前端视图两级重组 —— 能力来源页卡片内分两级:一级「能力主体」(active: skill/builtin/plugin/connector,默认展开)、二级「扩展项」(inactive: catalog/cache/agent,默认折叠灰显);卡片头加身份徽章(形态 `agent_form` / family)+ 构成行(`extension_breakdown`)。`extension_type` 仍不单占目录行(与 runtime_state/layer 重叠,防噪音)
