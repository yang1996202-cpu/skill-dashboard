...
**根因（历史）**：旧「一键诊断」流程的诊断缓存异步写入，覆盖了 fast-scan 的实时值。该 diagnose 流程已于 2026-06-28 整体移除，此根因不再可能复现；但通用防御原则仍成立——异步拿到的 totals 不应覆盖 fast-scan 的实时值。

**修复**：`scan.totals.skills` 只取 fast-scan 的值，任何异步结果都不覆盖它（代码里仍保留此防御）。
