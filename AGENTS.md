...
`POST /api/scan-run` 接受 `{directories, scope, checks}` 参数。`checks` 默认 `['same-name']`（**不含 upstream/content-changes**——upstream 烧 API 已迁「上游检测」视图;content-changes 变更 tab 已删;同名检测副产品出 `duplicates_identical` + 扫目录收 `structure_issues` 断链）。`scope` 控制目录范围:健康检测页可选 `current/active/review/all`(默认 active+review=用户自建+项目级;**不含 inventory 库存**——库存删不动/不该删,归能力来源页浏览)。
...
- **仪表盘顶部**(`renderStats`)：「资产规模」(skill 数 `global-stats.unique_skills` / 应用数 `targetGroups.length`)+「治理成果」(清理[hover 拆同名/副本/历史未分类,**不含断链**]+**断链**单独+更新+安装+同步+补来源+健康检测),数据来自 `/api/governance-stats`。label 前置根治旧版「值在前 label 在后」+裸标题「全局」的视觉错位;待处理存量(同名/损坏/上游过时)不上仪表盘,归各功能页 + badge
...
- **还要清 `duplicates_same_name`/`duplicates_identical`**:`_patch_scan_cache_remove` 删 (name,parent) → 从 duplicates 组移除该 location(剩余<2 解散组),否则删完副本 duplicates 没更新、界面还显示已删副本。
- **删除后界面刷新靠 patch cache + 重读,不靠 runScan（踩坑）**:`refreshIssuesAfterDelete` 旧版跑 `runScan`(几十秒,用户以为卡死;silent 失败静默用旧数据)。改:后端 `_patch_scan_cache_remove` 删后精准清 scan-result(含 duplicates),前端 `loadCachedScanResult` 重读 → 即时消失。
