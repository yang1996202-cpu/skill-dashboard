...
| `/api/governance-stats` | GET | 仪表盘「治理成果」聚合:`cleanup_total`+`cleanup_by_reason`(按 `move_to_trash.detail.reason` 分桶)+`update/install/copy/attach_total`(count 累加)+`scan_total`(整理次数)。reason 是 2026-07 埋点,历史无 reason 归 `uncategorized`(显示「历史未分类」) |
...
**删除 reason 埋点（治理成效用）**：上述 tab 删除 + `deleteAllBroken` + `deleteSelectedIssues` 都透传 `reason`（`deleteSkill` 第4参 / checkbox `data-sreason`），取值 `broken`/`same-name`/`identical`，写进 `move_to_trash.detail.reason`，喂 `/api/governance-stats` 的 `cleanup_by_reason`。2026-07 才加,历史删除无 reason 归 `uncategorized`(「历史未分类」)。
...
- **仪表盘顶部**(`renderStats`)：「资产规模」(skill 数 `global-stats.unique_skills` / 应用数 `targetGroups.length`)+「治理成果」(清理[hover 按 reason 拆损坏/同名/副本/历史未分类]+更新+安装+同步+补来源+整理),数据来自 `/api/governance-stats`。label 前置根治旧版「值在前 label 在后」+裸标题「全局」的视觉错位;待处理存量(同名/损坏/上游过时)不上仪表盘,归各功能页 + badge
