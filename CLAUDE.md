...
static/upstream.js          — 上游检测视图
.data/             — 运行时状态与缓存（state/、cache/，.gitignore）
docs/              — 项目文档（api-reference.md、troubleshooting.md、skill-model.md、source-recovery.md）
README.md
LICENSE
...
> 完整 API 表（含参数说明、body 格式、踩坑注释）见 **[docs/api-reference.md](docs/api-reference.md)**。
> 此处只列核心路由名和用途速查。改路由后必须同步更新 docs/api-reference.md。

| 端点 | 方法 | 作用 |
|---|---|---|
|  | GET | 当前 target skills 列表 |
|  | GET | 全部 skill 目录（按 Agent 分组，3 层缓存） |
|  | POST | 健康检测扫描（默认不含 upstream） |
|  | GET | 缓存的上次扫描结果 |
|  | GET | active-only 分类分布 |
|  | POST | 切换当前 target |
|  | POST | GitHub URL → 安装 skill |
|  | POST | npx skills CLI 安装 |
|  | GET | 导出单个 skill 为 zip |
|  | POST | 批量导出 skills 为 zip |
|  | POST | 导入 zip（base64）安装 skills |
|  | DELETE | 删除 skill（移入垃圾站） |
|  | GET | 上游版本检查 |
|  | PATCH | 从上游更新 |
|  | GET | 跨目录预览 SKILL.md |
|  | GET/DELETE | 垃圾站管理 |
|  | GET | by-author 分组轴数据 |
|  | GET | 跨 Agent MCP server 清单 |
|  | GET | 治理成效统计 |
|  | GET | 路由清单（调试） |
...
### 安装拦截 + 技能安装页（2026-07-08 独立）

装 skill 默认装**项目级目录**（`~/projects/<proj>/.<agent>/skills/`），装**全局根**（`~/.<agent>/skills/`、`~/.agents/skills/`）需确认弹窗。`confirmInstallGlobal` + `isGlobalSkillRoot` 在四个安装入口前判断。

**「技能安装」独立一级菜单**（2026-07-08）：从「当前目录技能」页拆出，左侧导航新增「安装」→「技能安装」。页面内：
- 顶部目标目录选择器（默认当前 target，切换同步全局）
- 双栏：从 GitHub 安装 + npx 安装
- 底部：拖拽 zip 导入（`POST /api/install/import`）

导出/导入改为 zip 格式（2026-07-08）：
- 单 skill 导出：卡片 ⬇ 按钮 → `GET /api/skill/{name}/export` → zip
- 批量导出：勾选后 → `POST /api/skill/export-batch` → 合并 zip
- 全部导出有 confirm 拦截（防误触）
- 导入：拖拽 zip → base64 → extract 到当前 target

原「当前目录技能」页「＋ 安装」改为跳转「技能安装」页快捷按钮。
...
### 前端数据流 + 当前目录技能页（2026-07-08 重排版）

- `loadData()` 只跑轻量 API（fast-scan、targets、global-stats）
- `/api/targets` 通过 `fetchTargets(force)` 读取，前端带 3 分钟 TTL 内存缓存
- `render()` 视图感知：只在激活视图时渲染对应内容
- 目录切换后乐观 `is_current` 更新 + 本地缓存同步

**「当前目录技能」页** 2026-07-08 改为卡片网格：
- 2 列卡片布局（窄屏自动单列），每卡含复选框 + 名称 + 描述 + 分类标签 + 导出按钮
- 分类标签**可点击筛选**（OR 逻辑），顶部显示全部分类色块（中文全称 + 数量）
- 搜索修复：`oninput="filterSkills()"`（函数不存在）→ `oninput="renderSkillsList()"`
- 「＋ 安装」按钮改为跳转「技能安装」页；导出/导入同理

### 目录视图抽象层

目录视图抽象定义在 `static/issues-cleanup.js`，但职责已拆分：

- `filterGroupsByView(groups, viewMode)`：按 `active`/`inventory`/`review`/`all` 过滤 Agent 分组，重新计算 `total_skills`，过滤空分组。**当前目录穿透兜底**(`predicate(t)||t?.is_current`)只对非 `active-user` 桶生效——切换进来的 project-local 等当前目录即便不在本视图桶也保留(避免"切了却在能力来源页消失")；但 agent 用户根(`active-user`,如 `~/.claude/skills`)**不穿透到库存/待核查**(2026-07-08 修正):它只属"当前可用",被兜底拽进 inventory/review 会裸露在 cache/catalog/project-local 之间,与视图语义冲突。all 视图不受影响
...
- **🔄 重新扫描按钮(2026-07-08)**:标题区第一排「＋ 添加路径」前加「🔄 重新扫描」按钮(`sources.js::rescanSources`),调 `fetchTargets(true)` 穿透三层缓存强制重扫盘(见下「/api/targets 缓存」)。打开页面默认吃缓存(最长 3min 不扫盘),用户点此按钮才实时刷新。数据流同 `loadData`:`targets=td.targets`、`targetGroups=td.groups`。
- **软链接农场识别(2026-07-08)**:`_classify_skill_dir_detail` 给目录标 `is_symlink_farm`——非隐藏条目≥2 且**全软链**(指向别的 agent 根的镜像,无独立能力)→ True。严格口径不误伤 `~/.claude/skills`(有真实目录)。实测 `~/.bob/skills`、`~/.augment/skills` 等 12+ 个外部工具广播建的软链农场命中,`~/.claude/skills`/`~/.agents/skills` 不命中。前端 `sourceFarmBadge(g)` 用 `g.dirs.every(t=>t.is_symlink_farm)`——只 group 内**所有**目录都是 farm 才标「镜像」徽章 + `src-card-farm` 灰显(opacity .72);CodeBuddy/Cursor/trae 等真实 agent 虽 user-skills 根是 farm,但 group 内还有 plugin/marketplace 非 farm 目录,不标。农场 group 仍默认折叠、不剔除出「N 个应用」计数,只标记+降级,不动文件。口径写进 `tests/test_governance_layer.py::TestSymlinkFarm`。
...
**三层缓存 + force 穿透（2026-07-08 修）**：`/api/targets` 数据经三层缓存——前端 `fetchTargets` 3min + 后端 `_targets_cache` 180s + discovery `_DISCOVER_SKILL_DIRS_CACHE` 60s。`?refresh=1`(force=true) 穿透前两层后，**`_list_targets` 把 `force=force_refresh` 透传给 `_discover_skill_dirs(force=)`**，第三层也跳过 → 三层全穿、强制重扫盘。能力来源页标题区「🔄 重新扫描」按钮(`sources.js::rescanSources`)调 `fetchTargets(true)` 实现一键实时刷新。修复前 force 穿不透 discovery 60s，强刷仍可能拿 60s 前旧结果。
...
**来源恢复（给 unknown skill 补上游）**：设计见 `docs/source-recovery.md`。blob/合集勾选/npx 安装 + code search 通用层 + 补来源入口已全部落地；**待做**：真实 GitHub Code Search 命中率实测。WorkBuddy/CodeBuddy 等 app 自管宿主 dashboard 只读旁观（steal 装进去实测可工作 + 留痕正常，与 app 版本管理并存的冲突未实测）。

**技能安装页增强**：导出/导入 zip 已落地；待做：导入支持拖拽文件夹（非 zip）、导入后自动分类。

**竞品调研与差异化**：skillslm、cc-switch 等同类工具的方法论对比。
