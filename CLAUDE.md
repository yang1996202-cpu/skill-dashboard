...
| `/api/source-aggregations` | GET | 按 owner/repo 聚合有来源的 skill(只收 category=user/project,复用 `detect_source_local` 三信号,0 GitHub API;3 分钟缓存,attach 补来源后失效);喂「按来源」分组轴视图 |
