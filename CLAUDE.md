...
`discovery.py` 排除了 `plugins/cache`（防 cache 灌水），Claude plugin 走 `load_claude_plugin_state` 在 `_list_targets` 单独注入：`enabled`→`runtime_state=enabled`→bucket `active-plugin`→当前可用；未启用→`installed`→`installed-disabled`→来源库存。`policy=observe`（plugin 走 `/plugin` 命令管理，不在 dashboard 删/切换）。前端 `activeCatOrder` 含 `installed-disabled`，否则 plugin 进了 active tier 却不渲染。
