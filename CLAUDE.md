...
- **插件按 marketplace 叠折叠父级**:`renderCatBlock`(sources.js)在能力桶内见到 ≥2 个带 `plugin_id`(`name@marketplace`)的插件目录时,按 marketplace 分区叠 `.mkt-arrow` 父级(`marketplaceOf(t)` 抽 `@` 后名)。Codex/Claude(插件 cache)+ buddy(plugins/skills/connectors marketplaces)统一走这套。配套两条规则:①**只藏真冗余**——buddy 容器目录(`external_plugins`/connectors/skills 容器,内容已在子插件/连接器枚举)由 inspector 标 `_buddy_hidden`,`_list_targets`(source.py)跳过;download 快照(`download_`+UUID 缓存名)重分类为 cache。②**聚合根不藏**——skills/connectors marketplace 顶层「Skill 市场汇总」「Connector 市场汇总」必须留可见,因为总数只在聚合根上、子目录不全枚举,藏掉就彻底丢总数(曾藏错,见 commit `8ce2596`)
